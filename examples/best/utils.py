...
def notalpha(s):
    for c in s.lower():
        if c not in 'абвгдежзийклмнопрстуфхцчшщъыьэюяё-':
            return True
    return False
...
class PoemTemplateLoader(object):
    ...
    def poem_to_template(self, poem_text):
        ...
        for line in poem_lines:
            ...
            line_tokens = [token for token in word_tokenize(line) if not notalpha(token) or token in ',.?!-:;']
...
class Word2vecProcessor(object):
    ...
    def __init__(self, w2v_model_file):
        ...
        self.lemmas = {}
        with open('data/lemmas.txt') as f:
            for line in f:
                w,l = line.rstrip('\n').split('\t', 1)
                self.lemmas[w] = l
        print('loaded lemmas', len(self.lemmas))
        
    def norm(self, w):
        return self.lemma2word.get(self.lemmas.get(w))
    
    def distances(self, vec, words):
        wl = [self.lemmas.get(w) for w in words]
        ww = [self.lemma2word.get(w) for w in wl]
        idx = {ww[i]:w for i,w in enumerate(words)}
        ww = [w for w in ww if w is not None]
        dists = self.word2vec.distances(vec, ww)
        pairs = [(idx[w],dists[i]) for i,w in enumerate(ww)]
